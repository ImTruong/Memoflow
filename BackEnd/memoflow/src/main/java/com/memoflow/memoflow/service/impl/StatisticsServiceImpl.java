...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    public OverviewStatsResponse getOverviewStats(UserPrincipal userPrincipal) {
        ...
        long grammarCount = userLessonProgressRepository.countCompletedLessonsByTypeToday(userId, startOfDay, "GRAMMAR_%");
        long listeningCount = userLessonProgressRepository.countCompletedLessonsByTypeToday(userId, startOfDay, "LISTENING_%");
    ...
    public ListeningStatsOverviewResponse getListeningOverviewStats(UserPrincipal userPrincipal) {
        ...
        long totalExams = userLessonProgressRepository.countCompletedLessonsByTypeToday(userId, LocalDateTime.of(2000, 1, 1, 0, 0), "LISTENING_%");
        long newExamsThisWeek = userLessonProgressRepository.countCompletedLessonsByTypeToday(userId, startOfWeek, "LISTENING_%");
        ...
        for (int i = 1; i <= 4; i++) {
            ...
            long completedCount = userLessonProgressRepository.countCompletedLessonsByTypeToday(userId, LocalDateTime.of(2000, 1, 1, 0, 0), typePattern);
            
            // Get recent exams (titles)
            org.springframework.data.domain.Pageable top3 = org.springframework.data.domain.PageRequest.of(0, 3);
            org.springframework.data.domain.Page<com.memoflow.memoflow.dto.response.ListeningLessonResponse> recentP = learningLessonRepository.findListeningLessons(userId, typePattern, "completed", top3);
            List<String> recentExams = recentP.getContent().stream().map(com.memoflow.memoflow.dto.response.ListeningLessonResponse::getTitle).collect(Collectors.toList());
            long totalPartCount = recentP.getTotalElements();
            long moreCount = Math.max(0, totalPartCount - recentExams.size());

            String percentage = totalExams == 0 ? "0%" : (int)((completedCount * 100.0) / totalExams) + "%";
    ...
    public GrammarStatsOverviewResponse getGrammarOverviewStats(UserPrincipal userPrincipal) {
        ...
        long totalLessons = userLessonProgressRepository.countCompletedLessonsByTypeToday(userId, LocalDateTime.of(2000, 1, 1, 0, 0), "GRAMMAR_PRACTICE");
        long newLessonsThisWeek = userLessonProgressRepository.countCompletedLessonsByTypeToday(userId, startOfWeek, "GRAMMAR_PRACTICE");
        ...
        for (LearningLesson practice : allPractices) {
            ...
            if (content != null) {
                ...
                if (idObj instanceof Number) grammarLessonId = ((Number) idObj).longValue();
                else if (idObj instanceof String) {
                    try { grammarLessonId = Long.parseLong((String) idObj); } catch (Exception e) {}
                }
                
                if (grammarLessonId == null) {
                    Object titleObj = content.get("grammarLessonTitle");
                    if (titleObj != null) grammarLessonId = lessonIdByTitle.get(titleObj.toString());
                }
            }
            if (grammarLessonId != null) {
                practicesByLessonId.computeIfAbsent(grammarLessonId, k -> new ArrayList<>()).add(practice);
            }
        }

        String[] colors = {"#F59E0B", "#10B981", "#3B82F6", "#8B5CF6", "#EC4899", "#F43F5E"};
        String[] icons = {"book-open-variant", "pencil-outline", "school-outline", "library-outline", "bookmark-outline"};

        List<GrammarStatsOverviewResponse.CategoryStats> categoryStats = new ArrayList<>();
        int i = 0;
        for (LearningLesson lesson : grammarLessons) {
            List<LearningLesson> practices = practicesByLessonId.getOrDefault(lesson.getId(), Collections.emptyList());
            if (practices.isEmpty()) continue;

            long completedCount = 0;
            List<GrammarStatsOverviewResponse.RecentLesson> finishedExams = new ArrayList<>();
            for (LearningLesson p : practices) {
                if (userLessonProgressRepository.existsByUserIdAndLearningLessonIdAndIsCompletedTrue(userId, p.getId())) {
                    completedCount++;
            ...
            String percentage = totalLessons == 0 ? "0%" : (int)((completedCount * 100.0) / totalLessons) + "%";
    ...
    public VocabularyStatsOverviewResponse getVocabularyOverviewStats(UserPrincipal userPrincipal) {
        ...
        long totalSetsLearned = userLessonProgressRepository.countCompletedLessonsByTypeToday(userId, LocalDateTime.of(2000, 1, 1, 0, 0), "FLASHCARD");
        long newWordsThisWeek = flashcardReviewRepository.countDistinctWordsReviewedToday(userId, startOfWeek);

        // Group by Categories (using PrivacyMode as simple categories for now, or extracted from content)
        // Actually, let's group by "Của tôi" and "Cộng đồng" as indicated by the user's flow
        
        List<VocabularyStatsOverviewResponse.CategoryStats> categories = new ArrayList<>();
        
        // Category 1: Mine
        List<UserLessonProgress> mineProgress = userLessonProgressRepository.findCompletedLessonsByUserIdAndType(userId, "FLASHCARD");
        List<UserLessonProgress> mySets = mineProgress.stream()
                .filter(p -> p.getLearningLesson().getCreator() != null && p.getLearningLesson().getCreator().getId().equals(userId))
        ...
        categories.add(VocabularyStatsOverviewResponse.CategoryStats.builder()
                ...
                .percentage(totalSetsLearned == 0 ? "0%" : (int)((mySets.size() * 100.0) / totalSetsLearned) + "%")
                .recentSets(mySets.stream().limit(3).map(p -> VocabularyStatsOverviewResponse.RecentSet.builder()
        ...
        categories.add(VocabularyStatsOverviewResponse.CategoryStats.builder()
                ...
                .percentage(totalSetsLearned == 0 ? "0%" : (int)((communitySets.size() * 100.0) / totalSetsLearned) + "%")
                .recentSets(communitySets.stream().limit(3).map(p -> VocabularyStatsOverviewResponse.RecentSet.builder()
                        .id(p.getLearningLesson().getId())
                        .title(p.getLearningLesson().getTitle())
                        .build()).collect(Collectors.toList()))
    ...
    @Override
    public java.util.List<VocabularyStatsOverviewResponse.RecentSet> getVocabularySetsByCategory(UserPrincipal userPrincipal, String category) {
        Long userId = userPrincipal.getId();
        List<UserLessonProgress> mineProgress = userLessonProgressRepository.findCompletedLessonsByUserIdAndType(userId, "FLASHCARD");
