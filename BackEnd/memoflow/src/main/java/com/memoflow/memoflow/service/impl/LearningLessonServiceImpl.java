...
public class LearningLessonServiceImpl implements LearningLessonService {
    ...
    @Override
    public StoryLessonResponse createStoryLesson(Long learningActivityId,
            CreateStoryLearningLessonRequest request, MultipartFile image, UserPrincipal userPrincipal) {
        LearningActivity activity = learningActivityRepository.getReferenceById(learningActivityId);
        ...
        LearningLesson savedLesson = learningLessonRepository.save(lesson);
        return mapToStoryLessonResponse(savedLesson);
    }

        @Override
        public StoryLessonResponse updateStoryLesson(Long id, UpdateStoryLearningLessonRequest request,
                        MultipartFile image, UserPrincipal userPrincipal) {
                LearningLesson lesson = learningLessonRepository.findById(id)
                                .orElseThrow(() -> new ResourceNotFoundException("Story Lesson", "id", id));

                if (!"TRUYEN_CHEM".equals(lesson.getType())) {
                        throw new ResourceNotFoundException("Story Lesson", "id", id);
                }
        ...
        @Override
        public void deleteStoryLesson(Long id, UserPrincipal userPrincipal) {
                LearningLesson lesson = learningLessonRepository.findById(id)
                                .orElseThrow(() -> new ResourceNotFoundException("Story Lesson", "id", id));
    ...
    public PageResponse<StoryLessonProgressResponse> getStoryLessons(UserPrincipal userPrincipal, Pageable pageable) {
        Page<LearningLesson> lessonPage = learningLessonRepository.findByType("TRUYEN_CHEM", pageable);
        ...
        List<StoryLessonProgressResponse> content = lessonPage.getContent().stream()
                .map(lesson -> mapToStoryLessonProgressResponse(lesson, progressMap.get(lesson.getId())))
                .collect(Collectors.toList());
    ...
    @Override
    @Transactional(readOnly = true)
    public StoryLessonProgressResponse getStoryLessonDetail(Long lessonId, UserPrincipal userPrincipal) {
        ...
        return mapToStoryLessonProgressResponse(lesson, progress);
    }

    @Override
    public void completeStoryLesson(Long lessonId, UserPrincipal userPrincipal) {
        LearningLesson lesson = learningLessonRepository.findById(lessonId)
                .orElseThrow(() -> new ResourceNotFoundException("Story Lesson", "id", lessonId));
    ...
    private StoryLessonProgressResponse mapToStoryLessonProgressResponse(
            LearningLesson lesson,
            UserLessonProgress progress) {
        StoryLessonResponse lessonResponse = mapToStoryLessonResponse(lesson);
    ...
    private StoryLessonResponse mapToStoryLessonResponse(LearningLesson lesson) {
        ...
        return StoryLessonResponse.builder()
                ...
                .learningActivityId(lesson.getLearningActivity() != null
                        ? lesson.getLearningActivity().getId()
                        : null)
                .build();
    }
