...
public interface WordRepository extends JpaRepository<Word, Long> {
    ...
    @Query("SELECT w FROM Word w WHERE w.learningLesson.id = :flashcardLessonId")
    ...
    @Query("SELECT w FROM Word w WHERE w.learningLesson.id = :flashcardLessonId AND LOWER(w.name) LIKE LOWER(CONCAT('%', :keyword, '%'))")
    Page<Word> findByFlashcardLessonIdAndName(@Param("flashcardLessonId") Long flashcardLessonId, @Param("keyword") String keyword, Pageable pageable);

    List<Word> findByNameContainingIgnoreCase(String name);
    @Query("SELECT COUNT(w) FROM Word w WHERE w.learningLesson.id = :flashcardLessonId")
    ...
    @Query(value = "SELECT w.* FROM words w " +
           ...
           "AND fr.id = (SELECT MAX(fr2.id) FROM flashcard_reviews fr2 WHERE fr2.word_id = w.id AND fr2.user_id = :userId)",
           ...
           "AND fr.id = (SELECT MAX(fr2.id) FROM flashcard_reviews fr2 WHERE fr2.word_id = w.id AND fr2.user_id = :userId)",
           nativeQuery = true)
    Page<Word> findDueWordsByLessonAndUser(@Param("flashcardLessonId") Long flashcardLessonId, @Param("userId") Long userId, @Param("now") LocalDateTime now, Pageable pageable);
