...
@NoArgsConstructor
@AllArgsConstructor
// DTO nhan du lieu tao moi truyen chem tu admin.
public class CreateStoryLearningLessonRequest {
    ...
    // Tieu de tieng Anh neu admin muon hien thi rieng.
    private String englishTitle;

    // Danh sach doan van, cac tu can highlight duoc dat trong cap ngoac {word}.
    @NotEmpty(message = "Paragraphs are required")
    private List<@NotBlank(message = "Paragraph must not be blank") String> paragraphs;

    // Danh sach tu vung trong truyen chem.
    @Valid
